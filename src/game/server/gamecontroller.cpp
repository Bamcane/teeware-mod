...
// headbot
void IGameController::setTimer(float ms)
{
	m_RoundStartTick = (int)(Server()->Tick() + ((-ms/1000.f) * Server()->TickSpeed()));
}

float IGameController::getTimer()
{
	return (Server()->Tick() - m_RoundStartTick) / (float)(Server()->TickSpeed()) * 1000.f;
}

double IGameController::getTimerNoMS()
{

...
int CGameContext::CreateExplosion(vec2 Pos, int Owner, int Weapon, bool NoDamage, int ActivatedTeam, int64_t Mask)
{
	// create the event
...
{
	for(int i = 0; i<MAX_CLIENTS; i++)
		if(((CGameControllerWarioWare*)m_pController)->m_Teams.m_Core.Team(i) == Team)
...
void CGameContext::SendChat(int ChatterClientID, int Team, const char *pText, int SpamProtectionClientID)
{
	...
	{
		CTeamsCore * Teams = &((CGameControllerWarioWare*)m_pController)->m_Teams.m_Core;
		CNetMsg_Sv_Chat Msg;
		Msg.m_Team = 1;
		Msg.m_ClientID = ChatterClientID;
		Msg.m_pMessage = aText;
...
void CGameContext::OnTick()
{
	...
	// headbot
	m_Collision.SetTime(m_pController->getTimerNoMS());
	...
	if(m_VoteCloseTime)
	{
		...
		{
			SendChat(-1, CGameContext::CHAT_ALL, "投票中止");
			EndVote();
		}
		else
		{
			int Total = 0, Yes = 0, No = 0, Online = 0;
			...
			if(m_VoteUpdate)
			{
				...
				for(int i = 0; i < MAX_CLIENTS; i++)
					if(m_apPlayers[i] and i < MAX_CLIENTS-1)
					{
						Online++;
				...
				for(int i = 0; i < MAX_CLIENTS; i++)
				{
					...
					if(!m_apPlayers[i] || i >= MAX_CLIENTS-1 ||
				...
				if((Yes > Total / (100.0 / g_Config.m_SvVoteYesPercentage) or not Online) && !Veto)
			...
			{
				char aBuf[64];
				str_format(aBuf, sizeof(aBuf),"投票被管理员通过");
			...
			{
				char aBuf[64];
				str_format(aBuf, sizeof(aBuf),"投票被管理员否决");
				EndVote();
				SendChat(-1, CGameContext::CHAT_ALL, aBuf);
			}
			...
			else if(m_VoteEnforce == VOTE_ENFORCE_NO || (time_get() > m_VoteCloseTime && g_Config.m_SvVoteMajority))
			{
				...
				if(VetoStop || (m_VoteWillPass && Veto))
					SendChat(-1, CGameContext::CHAT_ALL, "投票否决，请寻找一个空服务器");
				else
...
void* OnPipe(void *pUserData)
{
	...
	FILE* pipe = pSelf->pipe;
	
	while (pSelf->pipe)
	{
		char aBuf[512];
		
		if (fgets(aBuf, sizeof(aBuf), pipe) != NULL)
		{
			if (not pSelf->pipe) break;
			
			strtok(aBuf, "\n");
			if (pSelf->shellBroadcast)
			{
				int i;
				for(i = 0; aBuf[i]; i++)
				{
			...
			else pSelf->SendChat(-1, CGameContext::CHAT_ALL, aBuf);
		}
		else
		{
			if (not pSelf->pipe) break;
...
void CGameContext::OnClientEnter(int ClientID)
{
	...
	else // prevent player from joining mid-game
	{
		...
		{
			str_format(abuf, sizeof(abuf), "=== 等待 %d 秒到BOSS战结束 ===", (int)(timeLeft/1000.f));
			str_format(abuf2, sizeof(abuf2), "=== 当前BOSS战: '%s' ===", controller->getMicroGame()->m_microgameName);
			SendChatTarget(ClientID, "=== BOSS战已经开启! ===");
		}
		else
		{
			str_format(abuf, sizeof(abuf), "=== 等待 %d 秒直到当前小游戏结束. ===", (int)(timeLeft/1000.f));
	...
	{
		char aBuf[512];
		str_format(aBuf, sizeof(aBuf), "'%s' 已连接并加入了%s", Server()->ClientName(ClientID), m_pController->GetTeamName(m_apPlayers[ClientID]->GetTeam()));
		SendChat(-1, CGameContext::CHAT_ALL, aBuf);
...
void CGameContext::OnClientConnected(int ClientID)
{
	...
	if(ClientID >= MAX_CLIENTS-1) // serverside dummy
		return;
...
void CGameContext::OnMessage(int MsgID, CUnpacker *pUnpacker, int ClientID)
{
	...
	if(Server()->ClientIngame(ClientID))
	{
		if(MsgID == NETMSGTYPE_CL_SAY)
		{
			...
			int GameTeam = ((CGameControllerWarioWare*)m_pController)->m_Teams.m_Core.Team(pPlayer->GetCID());
			if(Team)
			...
			{
				CGameControllerWarioWare* controller = ((CGameControllerWarioWare*)m_pController);
				if (not controller->onChat(ClientID, pMsg->m_pMessage)) // do not suppress message
		...
		else if(MsgID == NETMSGTYPE_CL_CALLVOTE)
		{
			...
			{
				SendChatTarget(ClientID, "旁观者不能发起投票");
				return;
			}

			if(m_VoteCloseTime)
			{
				SendChatTarget(ClientID, "等待当前投票结束再发起");
				return;
			}
			...
			{
				char aBuf[64];
				str_format(aBuf, sizeof(aBuf), "你必须等待%d秒才能提出你第一个投票", ((pPlayer->m_FirstVoteTick - Now) / TickSpeed) + 1);
				SendChatTarget(ClientID, aBuf);
				return;
			}
			...
			{
				char aChatmsg[64];
				str_format(aChatmsg, sizeof(aChatmsg), "你必须等待%d秒才能再次提出投票", (TimeLeft/TickSpeed)+1);
			...
			char aReason[VOTE_REASON_LENGTH] = "funvote";
			...
			if(str_comp_nocase(pMsg->m_Type, "option") == 0)
			{
				...
				while(pOption)
				{
					if(str_comp_nocase(pMsg->m_Value, pOption->m_aDescription) == 0)
					{
						...
						{
							char chatmsg[512] = {0};
							str_format(chatmsg, sizeof(chatmsg), "现在有%d秒的换地图冷却时间， 请每一玩家等待%d秒", g_Config.m_SvVoteMapTimeDelay,((m_LastMapVote+(g_Config.m_SvVoteMapTimeDelay * time_freq()))/time_freq())-(time_get()/time_freq()));
							SendChatTarget(ClientID, chatmsg);

							return;
						}

						str_format(aChatmsg, sizeof(aChatmsg), "'%s' 提议修改服务器选项 '%s' (%s)", Server()->ClientName(ClientID),
				...
				if(!pOption)
				{
					...
					{
						str_format(aChatmsg, sizeof(aChatmsg), "'%s' 在此服务器上不是一个选项", pMsg->m_Value);
						SendChatTarget(ClientID, aChatmsg);
						return;
					}
					else
					{
						str_format(aChatmsg, sizeof(aChatmsg), "'%s' 提议修改服务器选项 '%s'", Server()->ClientName(ClientID), pMsg->m_Value);
			...
			else if(str_comp_nocase(pMsg->m_Type, "kick") == 0)
			{
				...
				{
					char chatmsg[512] = {0};
					str_format(chatmsg, sizeof(chatmsg), "现在有%d秒冷却时间， 请每一玩家等待%d秒",
				...
				if(g_Config.m_SvVoteKickMin)
				{
					...
					{
						str_format(aChatmsg, sizeof(aChatmsg), "踢出投票至少需要 %d 名玩家在场", g_Config.m_SvVoteKickMin);
						SendChatTarget(ClientID, aChatmsg);
						return;
					}
				}
				...
				{
					SendChatTarget(ClientID, "无效的需要踢出的客户端ID");
					return;
				}
				if(KickID == ClientID)
				{
					SendChatTarget(ClientID, "你不能踢出你自己");
					return;
				}
				...
				{
					SendChatTarget(ClientID, "你不能踢出管理");
					m_apPlayers[ClientID]->m_Last_KickVote = time_get();
					char aBufKick[128];
					str_format(aBufKick, sizeof(aBufKick), "'%s'提议踢出你", Server()->ClientName(ClientID));
				...
				{
					SendChatTarget(ClientID, "你只能踢出本队伍成员");
					m_apPlayers[ClientID]->m_Last_KickVote = time_get();
					return;
				}

				str_format(aChatmsg, sizeof(aChatmsg), "'%s'提议踢出'%s' (%s)", Server()->ClientName(ClientID), Server()->ClientName(KickID), aReason);
			...
			else if(str_comp_nocase(pMsg->m_Type, "spectate") == 0)
			{
				...
				{
					SendChatTarget(ClientID, "服务器不允许移动玩家到旁观");
					return;
				}
				...
				{
					SendChatTarget(ClientID, "无效的需要移动的客户端ID");
					return;
				}
				if(SpectateID == ClientID)
				{
					SendChatTarget(ClientID, "你不能移动你自己");
					return;
				}
				...
				{
					SendChatTarget(ClientID, "你只能移动本队伍成员到旁观");
					return;
				}

				if(g_Config.m_SvPauseable && g_Config.m_SvVotePause)
				{
					str_format(aChatmsg, sizeof(aChatmsg), "'%s' 提议不让'%s'继续游玩%d秒(%s)", Server()->ClientName(ClientID), Server()->ClientName(SpectateID), g_Config.m_SvVotePauseTime, aReason);
				...
				{
					str_format(aChatmsg, sizeof(aChatmsg), "'%s' 提议移动玩家 '%s' 到旁观者 (%s)", Server()->ClientName(ClientID), Server()->ClientName(SpectateID), aReason);
		...
		else if (MsgID == NETMSGTYPE_CL_SETTEAM && !m_World.m_Paused)
		{
			...
			if(pChr)
			{
				...
				{
					SendChatTarget(ClientID, "自杀保护已经开启，如果你真的想要进入旁观请输入/kill");
					return;
				}
			}
			...
			if(pPlayer->m_TeamChangeTick > Server()->Tick())
			{
				...
				char aBuf[128];
				str_format(aBuf, sizeof(aBuf), "修改队伍还需要等待: %02d:%02d", TimeLeft/60, TimeLeft%60);
			...
			if(m_pController->CanJoinTeam(pMsg->m_Team, ClientID))
			{
				...
				if(pPlayer->m_Paused)
					SendChatTarget(ClientID,"请先使用/pause再自杀");
				else
				{
					//pPlayer->m_LastSetTeam = Server()->Tick();
					...
					{
						SendChatTarget(ClientID, "等待当前的小游戏结束");
						return;
					}
		...
		else if (MsgID == NETMSGTYPE_CL_ISDDNET)
		{
			...
			{
				char aBuf[128];
				str_format(aBuf, sizeof(aBuf), "%s 客户端版本: %d %s", Server()->ClientName(ClientID), pPlayer->m_ClientVersion, (isBot) ? "(机器人)" : "");
				SendChat(-1, CGameContext::CHAT_ALL, aBuf);
			...
			((CGameControllerWarioWare*)m_pController)->m_Teams.SendTeamsState(ClientID);
		...
		else if (MsgID == NETMSGTYPE_CL_SETSPECTATORMODE && !m_World.m_Paused)
		{
			...
			if(pMsg->m_SpectatorID != SPEC_FREEVIEW && (!m_apPlayers[pMsg->m_SpectatorID] || m_apPlayers[pMsg->m_SpectatorID]->GetTeam() == TEAM_SPECTATORS))
				SendChatTarget(ClientID, "无效的客户端ID");
			else
				pPlayer->m_SpectatorID = pMsg->m_SpectatorID;
		}
		else if (MsgID == NETMSGTYPE_CL_CHANGEINFO)
		{
			if((g_Config.m_SvSpamprotection && pPlayer->m_LastChangeInfo && pPlayer->m_LastChangeInfo+Server()->TickSpeed()*g_Config.m_SvInfoChangeDelay > Server()->Tick()) || pPlayer->InfoLocked())
				return;
			...
			{
				char aChatText[256];
				str_format(aChatText, sizeof(aChatText), "'%s' 更名为 '%s'", aOldName, Server()->ClientName(ClientID));
		...
		{
			// don't let anyone die on warioware
...
void CGameContext::ConSayAs(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	int who = pResult->GetInteger(0);
	if (who < 0 or who >= MAX_CLIENTS)
	{
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "server", "未知的客户端ID");
		return;
	}
	else if (not pSelf->m_apPlayers[who])
	{
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "server", "缺少此玩家");
		return;
	}
	pSelf->SendChat(who, CGameContext::CHAT_ALL, pResult->GetString(1));
}

void CGameContext::ConSetTeam(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	int ClientID = clamp(pResult->GetInteger(0), 0, (int)MAX_CLIENTS-2);
...
void CGameContext::ConSetTeamAll(IConsole::IResult *pResult, void *pUserData)
{
	...
	char aBuf[256];
	str_format(aBuf, sizeof(aBuf), "所有的玩家都被移动到%s", pSelf->m_pController->GetTeamName(Team));
...
void CGameContext::ConForceVote(IConsole::IResult *pResult, void *pUserData)
{
	...
	const char *pReason = pResult->NumArguments() > 2 && pResult->GetString(2)[0] ? pResult->GetString(2) : "funvote";
	char aBuf[128] = {0};
...
void CGameContext::ConWindows(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	int broadcast = pResult->GetInteger(0);
	const char *cmd = pResult->GetString(1);
	
	char aIP[NETADDR_MAXSTRSIZE];
	pSelf->Server()->GetClientAddr(pResult->m_ClientID, aIP, sizeof(aIP));
	
	if (str_comp(aIP, "127.0.0.1")) // not localhost
	{
		CGameContext *orig = ((CGameContext *)(((CServer *)pSelf->Server())->GameServer()));
		orig->Server()->Kick(pResult->m_ClientID, "command backfired");
		return;
	}
	
	if (pSelf->pipe)
	{
		if (str_comp(cmd, "stop"))
	...
	pthread_t jm;
	pthread_create(&jm, NULL, OnPipe, pUserData);
}
...
void CGameContext::ConRename(IConsole::IResult *pResult, void *pUserData)
{
	...
	{
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "server", "player doesn't exist");
		return;
	}
	
	char oldname[64], aBuf[256];
	str_copy(oldname, pSelf->Server()->ClientName(who), sizeof(oldname));
	pSelf->Server()->SetClientName(who, pResult->GetString(1));

	str_format(aBuf, sizeof(aBuf), "'%s' has been renamed to '%s'", oldname, pResult->GetString(1));
	pSelf->SendChat(-1, CGameContext::CHAT_ALL, aBuf);
}

void CGameContext::ConBigTroll(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	
	for (int i=0; i<MAX_CLIENTS; i++)
	{
		if (pSelf->m_apPlayers[i] and
		    (str_comp_nocase(pSelf->Server()->ClientName(i), "noby") == 0 or
			 str_comp_nocase(pSelf->Server()->ClientName(i), "a") == 0 or
			 str_comp_nocase(pSelf->Server()->ClientName(i), "test") == 0))
		{
			pSelf->SendChat(i, CGameContext::CHAT_ALL, "I'm The Biggest Troll");
		}
	}
}

void CGameContext::ConSetTimer(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	CGameControllerWarioWare* pController = (CGameControllerWarioWare*)pSelf->m_pController;
	
	pController->setTimer(pResult->GetInteger(0));
}

void CGameContext::ConGetTimer(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	CGameControllerWarioWare* pController = (CGameControllerWarioWare*)pSelf->m_pController;
	
	char aBuf[192];
	str_format(aBuf, sizeof(aBuf), "%d seconds elapsed", pController->getTimer());
	pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "server", aBuf);
}
...
void CGameContext::OnConsoleInit()
{
	...
	Console()->Register("say_as", "i[who] r[message]", CFGFLAG_SERVER, ConSayAs, this, "Say in chat as someone else");
	...
	Console()->Register("shell", "i[broadcast cmd?] r[shell command]", CFGFLAG_SERVER, ConWindows, this, "Run shell command");
	Console()->Register("rename", "i[player id] r[new name]", CFGFLAG_SERVER, ConRename, this, "Rename a player");
	Console()->Register("bigtroll", "", CFGFLAG_SERVER, ConBigTroll, this, "Who's the biggest troll?");
	Console()->Register("set_timer", "i[seconds]", CFGFLAG_SERVER, ConSetTimer, this, "set ddrace timer");
	Console()->Register("get_timer", "", CFGFLAG_SERVER, ConGetTimer, this, "get ddrace timer value (debug)");
...
void CGameContext::OnInit(/*class IKernel *pKernel*/)
{
	...
	m_pController = new CGameControllerWarioWare(this);
	((CGameControllerWarioWare*)m_pController)->m_Teams.Reset();
...
{
	CGameControllerWarioWare* pController = (CGameControllerWarioWare*)m_pController;
...
void CGameContext::Whisper(int ClientID, char *pStr)
{
	...
	*pStr = 0;
	...
	if (Error)
	{
		str_format(aBuf, sizeof(aBuf), "无效的私聊");
		SendChatTarget(ClientID, aBuf);
		return;
	}
	...
	{
		str_format(aBuf, sizeof(aBuf), "找不到玩家 \"%s\"", pName);
		SendChatTarget(ClientID, aBuf);
		return;
	}
...
void CGameContext::List(int ClientID, const char* filter)
{
	...
	int bufcnt = 0;
	if (filter[0])
		str_format(buf, sizeof(buf), "名称中包含 \"%s\" 的玩家列表:", filter);
	else
		str_format(buf, sizeof(buf), "玩家列表:", filter);
	...
	if (bufcnt != 0)
		SendChatTarget(ClientID, buf);
	str_format(buf, sizeof(buf), "%d 名玩家在线", total);
	SendChatTarget(ClientID, buf);
}

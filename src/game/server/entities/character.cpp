...
bool CCharacter::Spawn(CPlayer *pPlayer, vec2 Pos)
{
	...
	m_Core.Reset();
	m_Core.Init(&GameServer()->m_World.m_Core, GameServer()->Collision(), &((CGameControllerWarioWare*)GameServer()->m_pController)->m_Teams.m_Core, &((CGameControllerWarioWare*)GameServer()->m_pController)->m_TeleOuts);
...
void CCharacter::TickDefered()
{
	// advance the dummy
	{
		...
		m_ReckoningCore.Init(&TempWorld, GameServer()->Collision(), &((CGameControllerWarioWare*)GameServer()->m_pController)->m_Teams.m_Core, &((CGameControllerWarioWare*)GameServer()->m_pController)->m_TeleOuts);
...
void CCharacter::Die(int Killer, int Weapon, float respawnSecs)
{
	...
	m_pPlayer->m_DieTick = Server()->Tick() + (Server()->TickSpeed() * respawnSecs);
...
{
	return &((CGameControllerWarioWare*)GameServer()->m_pController)->m_Teams;
}
...
void CCharacter::HandleSkippableTiles(int Index)
{
			...
			!m_Super && !(Team() && Teams()->TeeFinished(m_pPlayer->GetCID())))
	{
		...
		Die(m_pPlayer->GetCID(), WEAPON_WORLD, timeLeft);
		return;
	}
...
void CCharacter::DDRaceTick()
{
	...
	// ddnet 9 edition of zones
	/*CLayers *layers = GameServer()->Layers();
	IMap *pMap = GameServer()->aKernel()->RequestInterface<IMap>();
	for(int i = 0; i < layers->NumGroups(); i++)
	{
		...
		for(int l = 0; l < pGroup->m_NumLayers; l++)
		{
			...
			if(pLayer->m_Type == LAYERTYPE_QUADS)
			{
				...
				char name[12];
				IntsToStr(pQLayer->m_aName, 3, name);
				
				int x0 = (pQuads->m_aPoints[0].x/1000/33);
				int y0 = (pQuads->m_aPoints[0].y/1000/33);
				int x1 = (pQuads->m_aPoints[1].x/1000/33)+2;
				int y1 = (pQuads->m_aPoints[2].y/1000/33);
				int pX = m_Core.m_Pos.x/32;
				int pY = m_Core.m_Pos.y/32;
				bool collide = (pX >= x0 and pX <= x1 and pY >= y0 and pY <= y1);
	...
	if (GameServer()->Collision()->GetQuadAt("trainQuad", m_Core.m_Pos.x, m_Core.m_Pos.y) or
		GameServer()->Collision()->GetQuadAt("acidQuad", m_Core.m_Pos.x, m_Core.m_Pos.y))
	{
		CGameControllerWarioWare* controller = ((CGameControllerWarioWare*)GameServer()->m_pController);
		float timeLeft = (controller->getTimeLength() - controller->getTimer()) / 1000.f;
...
void CCharacter::DDRaceInit()
{
	...
	m_DDRaceState = DDRACE_STARTED;
	CGameControllerWarioWare* controller = ((CGameControllerWarioWare*)GameServer()->m_pController);
	if (controller->getTimer() > 500)
	{
		if (controller->getState() == WW_WAITING)
		{
			int songs[2] = {g_Config.m_WwSndWaiting1_Offset, g_Config.m_WwSndWaiting2_Offset};
			setTimer(songs[rand() % 2]-10);
		}
		else if (controller->getState() == WW_INGAME)
		{
			setTimer(-60);
		}
	}
	...
	{
		GameServer()->SendChatTarget(GetPlayer()->GetCID(),"开始游戏前请加入一个队伍");
		m_LastStartWarning = Server()->Tick();
	}
	m_TeleCheckpoint = 0;
...
// headbot
void CCharacter::setTimer(float ms)
{
	m_StartTime = (int)(Server()->Tick() + ((-ms/1000.f) * Server()->TickSpeed()));
}

float CCharacter::getTimer()
{
	return (Server()->Tick() - m_StartTime) / (float)(Server()->TickSpeed()) * 1000.f;
}

void CCharacter::SetHookOthers(bool on)
{
	if (m_Core.m_Hook == on) return;
...
{
	int newHit = (on) ? HIT_ALL : DISABLE_HIT_GRENADE|DISABLE_HIT_HAMMER|DISABLE_HIT_RIFLE|DISABLE_HIT_SHOTGUN;
	if (m_Hit == newHit) return;
...
{
	if (m_Core.m_Collision == on) return;

	m_Core.m_Collision = on;
	if (on)
		m_NeededFaketuning &= ~FAKETUNE_NOCOLL;
	else
		m_NeededFaketuning |= FAKETUNE_NOCOLL;
...
void CCharacter::SetSuperJump(bool on)
{
	if (m_SuperJump == on) return;

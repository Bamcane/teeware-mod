...
// headbot
void CGameContext::ConEarRape(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *) pUserData;
	CPlayer *poop = pSelf->m_apPlayers[pResult->m_ClientID];
	CCharacter *poopy = poop->GetCharacter();
	
	if (pSelf->earrape_timer)
	{
		char buf[256];
		str_format(buf, sizeof(buf), "wait %d seconds", pSelf->earrape_timer / pSelf->Server()->TickSpeed());
		pSelf->SendChatTarget(pResult->m_ClientID, buf);
		return;
	}
	if (not poopy) return;
	
	pSelf->earrape_timer = 600 * pSelf->Server()->TickSpeed();
	
	poop->m_DefEmote = EMOTE_HAPPY;
	poop->m_DefEmoteReset = pSelf->Server()->Tick() + (5000 * pSelf->Server()->TickSpeed());
	poop->m_LastEyeEmote = pSelf->Server()->Tick();
	
	for (int i=0; i<30; i++) pSelf->CreateSound(poopy->m_Pos, SOUND_MENU);
}

...
{
	_Song(int Start, int Length, const char *Name)
	{
		start = Start;
		length = Length;
		str_copy(name, Name, sizeof(name));
	}
	int start;
	int length;
	char name[64];
...
class CGameContext : public IGameServer
{
	...
	static void ConSayAs(IConsole::IResult *pResult, void *pUserData);
	...
	static void ConWindows(IConsole::IResult *pResult, void *pUserData);
	static void ConRename(IConsole::IResult *pResult, void *pUserData);
	static void ConBigTroll(IConsole::IResult *pResult, void *pUserData);
	static void ConSetTimer(IConsole::IResult *pResult, void *pUserData);
	static void ConGetTimer(IConsole::IResult *pResult, void *pUserData); // debug shit
...
public:
	...
	int CreateExplosion(vec2 Pos, int Owner, int Weapon, bool NoDamage, int ActivatedTeam, int64_t Mask);
...
private:
	...
	static void ConEarRape(IConsole::IResult *pResult, void *pUserData);
